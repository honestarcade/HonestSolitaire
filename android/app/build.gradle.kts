...
// which would match an unanchored grep: a debug-signed bundle reported as
// upload-signed, GATE PASSED, exit 0. The same variable would make a correct
// build fail. `_JAVA_OPTIONS` behaves identically, and any future log line
// containing the phrase has the same effect. tools/gate.sh reads this file
// back rather than the log, which is what closes that hole.
//
...
android {
    // Immutable after the first Play upload. tools/check_aab.sh and the
    // workflows name the same id; keep them in sync.
    namespace = "com.honestarcade.solitaire"
    ...
    defaultConfig {
        applicationId = "com.honestarcade.solitaire"
    ...
    buildTypes {
        release {
            if (hsSigningComplete) {
                ...
                // set" can drift apart (ASCII whitespace vs. Kotlin's
                // isNullOrBlank, for one); a disagreement is now a gate
