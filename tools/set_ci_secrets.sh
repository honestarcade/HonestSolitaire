...
#
# It PARSES that file rather than sourcing it, deliberately. make_upload_key.sh
# writes it with a printf block specifically so sourcing is inert even if a
# password holds shell metacharacters -- but parsing is the supported,
# tested path regardless, so this script never depends on that safety net
# holding for every future edit to the writer.
#
...
# Must match make_upload_key.sh's APP_SLUG.
APP_SLUG="${HS_APP_SLUG:-solitaire}"
# Must match make_upload_key.sh's HS_SECRETS_DIR, if set.
...
REPO="honestarcade/HonestSolitaire"
