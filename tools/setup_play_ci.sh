...
APP_SLUG="${HS_APP_SLUG:-honestsolitaire}"
APP_DISPLAY_NAME="${HS_APP_DISPLAY_NAME:-Honest Solitaire}"
PROJECT="$APP_SLUG-ci"
SA_NAME="$APP_SLUG-ci"
SA_EMAIL="$SA_NAME@$PROJECT.iam.gserviceaccount.com"
# Override with HS_SECRETS_DIR to keep secrets elsewhere.
SECRETS_DIR="${HS_SECRETS_DIR:-$HOME/HonestArcadeApps/secrets}"
KEY_PATH="$SECRETS_DIR/$APP_SLUG-ci.json"
...
if gcloud projects describe "$PROJECT" > /dev/null 2>&1; then
  ...
  gcloud projects create "$PROJECT" --name="$APP_DISPLAY_NAME CI" --quiet
fi
...
if gcloud iam service-accounts describe "$SA_EMAIL" --project "$PROJECT" > /dev/null 2>&1; then
  ...
  gcloud iam service-accounts create "$SA_NAME" \
    --project "$PROJECT" \
    --display-name "$APP_DISPLAY_NAME CI" \
    --quiet
fi
...
echo "    $SA_EMAIL"
echo "  with app-level permissions for $APP_DISPLAY_NAME:"
